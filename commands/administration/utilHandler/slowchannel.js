...
module.exports = async (message, msg, lang, client) => {
    ...
    const embedSlowChannel = new MessageEmbed()
        .setTitle(client.languages.__({phrase: 'slowchannel.embedslowchannel_title', locale: lang}))
        .setColor('RED')
    const embedSlowON = new MessageEmbed()
        .setTitle(client.languages.__({phrase: 'slowchannel.embedSlowON_title', locale: lang}))
        .setColor('RED')
    const rowSlowON = new MessageActionRow()
    .addComponents(
        new MessageButton()
        .setCustomId('slowmodecancel')
        .setLabel(client.languages.__({phrase: 'slowchannel.cancel', locale: lang}))
        .setStyle('DANGER'),
    )
    const rowSlowChannel = new MessageActionRow()
        .addComponents(
            new MessageButton()
                ...
                .setLabel(client.languages.__({phrase: 'slowchannel.on', locale: lang}))
                .setStyle('SUCCESS'),
            new MessageButton()
                .setCustomId('slowmodeoff')
                .setLabel(client.languages.__({phrase: 'slowchannel.off', locale: lang}))
                .setStyle('DANGER'),
            )
            ...
            if(i.user.id === message.author.id){
                if (i.customId === 'slowmodeoff') {
                    ...
                    const embed = new MessageEmbed()
                        .setTitle(client.languages.__({phrase: 'slowchannel.setoff', locale: lang}))
                        .setColor('GREEN');
                ...
                if (i.customId === 'slowmodeon') {
                    ...
                    slowTimeCollector.on('collect', async m => {
                        ...
                        if (!toSecond || toSecond == undefined) {
                            ...
                            const embed = new Discord.MessageEmbed()
                                ...
                                .setDescription(client.languages.__({phrase: 'slowchannel.validformat', locale: lang}))
                        ...
                        const embed = new MessageEmbed()
                            .setTitle(client.languages.__({phrase: 'slowchannel.seton', locale: lang}))
                            .setColor('GREEN');
                        message.channel.send({embeds: [embed]})
                    })  
                    slowTimeCollector.on('end', collected => {
                        if(collected.size == 0){
                            ...
                            const embed = new MessageEmbed()
                                .setAuthor(client.languages.__({phrase: 'embederror.title', locale: lang}))
                                .setDescription(client.languages.__({phrase: 'slowchannel.expiredinteraction', locale: lang}))
                                .setThumbnail("https://2.bp.blogspot.com/-CPO_z4zNSnc/WsY667p0JgI/AAAAAAAAYRs/ubTMJD5ToyImbR-o4EiK18gBypYXd0RiwCLcBGAs/s1600/Mercenary%2BGarage%2BError%2BGIF.gif")
                                .setColor("RED")
                            return message.channel.send({embeds: [embed]})
                        }
