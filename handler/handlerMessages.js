    ...
    client.on("messageCreate", async (message) => {
        ...
        if(!channel.permissionsFor(message.guild.me).has("USE_EXTERNAL_EMOJIS")){
            const embed = new MessageEmbed()
                ...
                .setDescription(`**I need \`USE_EXTERNAL_EMOJIS\` permissions to execute most commands, before using me please give me these permissions.**`)
        ...
        if(!channel.permissionsFor(message.guild.me).has("EMBED_LINKS")){
            const embed = new MessageEmbed()
                .setAuthor(`❌ ¡ There's a mistake !`)
                .setDescription(`**I need \`EMBED_LINKS\` permissions to execute most commands, before using me please give me these permissions.**`)
                .setThumbnail("https://2.bp.blogspot.com/-CPO_z4zNSnc/WsY667p0JgI/AAAAAAAAYRs/ubTMJD5ToyImbR-o4EiK18gBypYXd0RiwCLcBGAs/s1600/Mercenary%2BGarage%2BError%2BGIF.gif")
                .setColor("RED")
            return message.channel.send({embeds: [embed]});
        }
